...
class RailNetwork:
    ...
    def simple_segment_from_endpoint(
        ...
        ⚠ **已知盲区（2026-09-10 登记，用户拍板"不修"）**：本函数只按**度数**
        行走，**不看 `turn_allowed`**——若一条链内部存在"过境转向不许可"的
        二度节点（例如手工搭出的 90° 急折、发卡弯），它会把**走不通的链**
        当成一整段返回，于是 `total_length` 被**高估**。受影响的是调用方
        两处既有判断：**折返可行性**（"这段够不够放下车身"）与信号的
        **"安全停车位置"**（车身是否会跨道岔）。当前存档上此类节点 = 0，
        故不咬人；**不修**（改它会动折返/信号这两块已人工验收的区域）。
        **通用准则（用户口径）**：**分段宁可更激进——多切总是安全的，
        少切才会把走不通的链当成一段**。需要"段内保证可通行"的场合
        （计划层）请用 `model/segments.py` 的分区，它按"度数 ≠ 2 **或**
        过境转向不许可"切分；回归对照见 `tests/test_segments.py` ②。
        """
