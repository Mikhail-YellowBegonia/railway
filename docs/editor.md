...
| `K`（悬停其它列车车尾） | 冻结“前往连挂”并追加；目标列车必须停放 |
| `W` | 追加“等待连挂” |
| `Delete` / `Ctrl+Delete` | 删除当前条目 / 清空当前胜出控制车的计划 |
| `Esc` | 丢弃当前草稿，并与 `P` 一样先冻结“末条→第一条”闭环；闭环失败则保持编辑态 |

纯 `goto` 计划退出前冻结“末条→第一条”闭环；含 `goto_couple` 或 `wait_couple` 的计划
确认成一次性事件链，最后一条完成后停车，不生成闭环。连续创建条目时，下一条从上一条
已确认终点构造，而不是从屏幕上仍停放的列车位置构造。
