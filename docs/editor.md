...
### 1.1.2 PLAY 计划编辑叠加态（P5）

选中一列完全停放且含控制车的列车，在 PLAY 中按 `P` 进入；编辑期间暂停该列车的
计划投影，便于连续确认往返条目。再次按 `P` 退出后，已确认条目开始执行。

| 操作 | 行为 |
|---|---|
| 左键点节点 | 按序追加硬约束锚点 |
| 道岔锚点后左键点其相邻边 | 将最后锚点升级为控制点，指定离开边 |
| 左键点轨道 | 设置终点并显示编辑期候选路径；不可达时红色显示并打印原因 |
| `Backspace` | 先撤销终点，再逐个撤销锚点 |
| `Enter` | 将候选路径冻结为 `FixedRoute`，追加到胜出控制车计划 |
| `K`（悬停轨道 edge 或外部端头） | 冻结到该 edge 的“前往连挂”；执行时按进入方向取首个可达端头。允许在目标仍属于本编组时预建 headshunt selector |
| `K`（悬停本列车内部车钩） | 追加“从逻辑车头后第 n 位解挂” |
| `R` | 追加 strict `simple_segment` 折返命令 |
| `W` | 追加“等待连挂” |
| `X` / `Ctrl+X` | 删除当前条目 / 清空当前胜出控制车的计划 |
| `Esc` | 丢弃当前草稿，并与 `P` 一样先冻结“末条→第一条”闭环；闭环失败则保持编辑态 |

纯 `goto` 计划退出前冻结“末条→第一条”闭环。含连挂/解挂/折返动作的固定场景调车计划
不生成空间 `loop_route`，但条目指针仍循环回绕；运行期只消费已冻结路线，不重新寻路。
连续创建条目时，下一条从上一条已确认终点构造；遇到 `reverse` 后构造方向翻转。

PLAY 中按 `O` 可打开基础调度计划选单。当前版本只读，显示胜出控制车、循环模式、
当前条目和完整条目列表，为后续计划选择与操作预留稳定入口；`O` 或 `Esc` 关闭。

有计划的焦点列车按空格切换自动驾驶暂停/继续。暂停会立即停车、清除当前计划投影，
但不移动计划指针或改写冻结路线；调度器在暂停期间不会重新抢回控制。再次按空格后从
实际停车位置继续消费当前冻结路线。该状态仅存在于运行期，不落盘。无计划列车的空格
仍是普通紧急停车。游戏窗口不再显示终端输出镜像。

### 1.1.3 控制车配置（P7 验收辅助）

PLAY 中选中列车后按 `I` 打开编组面板。地图上控制车以青色驾驶台方框标记，当前遴选
胜出者另有黄色外环；面板显示每节的控制标志、优先级与胜出者。

列车完全停放且未处于计划编辑时，面板内可用 `1`–`9` 选择车厢、`[`/`]` 调整优先级、
`C` 切换控制车标志。运行中或编辑计划时修改会被拒绝，防止执行权在行驶中切换。

面板同时列出胜出控制车的命令及当前指针；编辑计划时可用 `X` 删除当前条目，或用
`Ctrl+X` 清空。删除/追加普通 `goto` 条目会使旧闭环失效，因此退出编辑态时必须重新
冻结闭环；含动作条目的固定场景循环则直接确认退出。

### 1.1.4 手动编组与折返

不在计划编辑时，PLAY 中悬停内部车钩按 `K`、空格或回车可解挂；悬停其它停放列车端头按
`K` 可直接连挂，未贴住时会先下达临时接近指令并在到位后连挂。选中完全停放的列车按 `R`
可原地折返（逻辑首尾互换，车厢物理姿态不变）；车身不在同一 strict
`simple_segment` 时会拒绝。计划编辑期间在轨道 edge 或外端上按 `K` 追加“前往连挂”
（本车外端也可作为 edge 快捷来源，不绑定当前车厢），内钩 `K` 追加解挂，`R` 追加折返命令。
...
拓扑的最短路径查找，实现困难，暂时逃避。相关空间/拓扑加速见 §11（空间索引）。
...
- `Backspace` 撤销最后一个草稿成员；悬停既有 POI 显示名称、类型和成员数，`X`
...
**当前状态**：编辑器基础能力已完成并服务于 P9 demo 收口。本文件后续内容主要保留
历史设计与实现依据；新的开发顺序以 `docs/roadmap.md` 和
`docs/progress_snapshot.md` 为准，不再把进阶平行吸附列为当前近期重点。
...
> - **瓦片化数据结构** → ✅ **已实施**（2026-09-10 更正）：它就是现在的
>   `model/spatial_index.py`（uniform grid，`TILE_SIZE=50m`，340× 加速，
>   `I` 键可视化），详见 §11、§7。此处原写"设计研讨稿见 `docs/tiling.md`
>   （未实施）"**两处都有误**：设计稿 `docs/tiling.md` **确实存在过**
>   （`7332f63` 创建），并在实施完成后被**主动删除**（`b8c9209`"归档空间索引
>   实施完成，清理临时文件"）。⚠ 该文件已不存在，但 `model/spatial_index.py` /
>   `model/rail_network.py` 的注释与本文 §3.4 仍在引用它（悬空引用，待清理）。
> - **LOD** → ⚠ **demo 不做**（2026-09-10 拍板："没有出现性能瓶颈那就不管"；
>   代码里零实现，与瓦片化不是同一件事——瓦片化已落地）。
