...
大致Roadmap：
A. 寻路算法微调，支持在轨道尽头掉头，支持将Edge上的任意点作为寻路起止点。
B. 实现PID控制器，整合进手动控制中
C. 实现列车自动驾驶与停车，替代手动控制，这对后续测试有益
...
## C 自动驾驶
列车按照玩家点出的位置，自动驾驶并停在目标处
并没有很多新增功能，而是对之前的复用：
1. 点选目标可使用编辑器既有snap方法，支持Edge途中和Node
2. 寻路方法没有改变，A阶段已经针对Edge途中的目标进行了适配，列车收到命令后按需寻路并创建Path
3. 行驶过程中向PID控制器传入预期速度
