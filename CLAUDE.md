...
——**✅ 设计已研讨完毕、规划已封口（2026-09-10）**；**P0（分段器）/ P1（计划数据类型）/ P2（锚点解析器）/ P3a（冻结路径）/ P4（拓扑变更防火墙）已落地，
下一步 = P3b（计划机制接入列车——第一个"有行为"的阶段）→ P5（最小交互）**。
...
- **引用与保护**：引用落在**物理要素（节点/边）**；"段（simple_segment）"只作
  **派生视图**（不进存档、不留 id）；**被引用的要素禁止删除**；
  **切分（放信号 / 建造 Path-snap 截断）放行，但自动把引用机械改写映射到子边**。
- **demo 命令集合 = `goto` / `goto_couple` / `wait_couple`**，**无跳转命令**；
  循环 = 走完回绕第一项；**一次到达事件最多走一圈**，整圈无可执行 ⇒ 停车等待并提示；
  步进语义表见 `plan_layer_roadmap.md` §2.3（含 R10 的连挂侧强制步进）。
- **失效语义**：引用对象**不存在** ⇒ 跳过该条目（计划保留；**GUI 可见 + 标红 + 原因
...
- **demo 要做（最大缺口）**：**计划式自动驾驶**——创建计划并跟随计划。现状是
  ...
  「计划层定案」；**实施路线 = `docs/plan_layer_roadmap.md`（P0~P9）——P0~P4 已完成，下一步 P3b**；
...
## 数据结构重想：以车厢为中心（**✅ Q1~Q23 已全部拍板；规划已封口**）

用户 2026-09-10 看完上面的审计结论后判断"现有数据结构不太稳固，建议重新思考"，
提出四点设想，完整记录 + 差距分析 + Q1~Q23 研讨问题 + **§6 逐字段归属草案** +
**§8 可无感落地提案**见
**`docs/wagon_centric_data.md`**（**结论已定稿**；§9 风险登记为长期台账）。
要点（**Q1/Q2/Q6/Q8/Q11 已拍板**；**计划层形态见上「计划层定案」**）：
...
- **Q7 数据流是单向管道**：**调度计划 → 寻路需求 → 寻路结果 → 预约路径 → 物理模拟
  ...
  只缺最上游的"调度计划"那一段。
**仍待研讨**：~~Q12~~（**已降级为纯措辞问题，不阻塞**）、~~§9 风险登记 R1~R9~~ ⇒
**2026-09-10 已封口**：**Q1~Q23 全部拍板**；风险登记 **R1 已按"拆分"落地、
R9 因"计划不落盘"延后、R10 已定并写进语义表、R11 押到 P3 与遴选一起加断言**，
剩余 R2~R8 为长期/纪律类（各有触发时机）。
**⇒ 计划层规划已封口，可进入实现（`docs/plan_layer_roadmap.md` P0~P9）。**
...
- ✅ **额外落地（2026-09-10，用户选定方案）**：**`WagonConfig`（只读配置）+
  `Wagon`（运行时对象）拆分** —— `Wagon` 持有 `config` + `data_log`，只读属性
  **委托**给 `config`（读代码无需区分二者），并带 **`tick(dt)`（空实现、未接线）**；
  `Consist.wagons` 统一为 `list[Wagon]`，`Consist.__post_init__` 自动包装裸
  `WagonConfig`（既有构造点不用改）；新增便捷工厂 `create_simple_car()`。
  **接线在任务 4**，必须遵守：**物理不下放**（位置真值仍是编组单一 `TrainState.s`）、
  **由编组遍历自己的车厢**调用（顺序 = 车头→车尾，天然确定）、
...
- **T3 仍未做**：~~只读冻结~~（**Q3 已撤销——"只读"是设计准则、不做强制**）、
  ...
  `create_simple_car()`——**它们各自的接线时机已定**：`control_cars()` 与
  `have_control`/`priority` 在 **P3**（遴选）接线；`Wagon.tick()` **demo 不接线**（Q23-9）；
  `create_simple_car()` 只在测试/工厂处使用。（接线时须遵守上面的 tick 约定。）
