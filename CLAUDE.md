...
**Q6 补充（2026-09-10，OpenTTD decouple 分支参照）**：计划 = **有序命令列表 +
指令指针**；存在「**等待连挂**」命令，执行它的车即**挂起**；控制权的得失靠
**连挂/解挂事件信号**促使控制车**步进**后续命令（示例：1 前往A点 → 2 等待连挂 →
3 空命令（解挂后跳转至此）→ 4 前往B点；低优先级控制车在 A 点挂车失去控制权、
被解挂后收回控制权，需事件把它从命令 2 推到命令 3）。⇒ 落选计划**不搬移/不丢弃**，
"**挂起**"的形态是"**指令指针停在等待命令上**"。由此新增 **Q13**（事件有哪些、
谁发出、**车厢没有 tick 如何接收**、指令指针归谁——倾向归车厢 A 桶待确认）。
