...
提出四点设想，完整记录 + 差距分析 + Q1~Q12 研讨问题 + **§6 逐字段归属草案**见
**`docs/wagon_centric_data.md`（研讨稿）**。要点（**未定稿**；Q1/Q2/Q6/Q8 已拍板）：
...
这不是取舍而是 Q1/Q2 的推论。推论（**待确认**）：连挂时"落选计划"不存在丢弃问题，
计划始终留在其控制车上，遴选只决定"此刻执行谁的"，故计划类型只需运行期的
"被选中/未被选中"标记，不需要"挂起/恢复"状态。
...
字段逐条判了归属，并新暴露 Q11（POI 引用完整性）、Q12（编组构成落盘）。

⚠ **Q3/Q4/Q5/Q7/Q9 细节/Q10/Q11/Q12 仍未定**（只读如何强制、载货更新由谁驱动、
priority 与 id 决断方向、计划→route 的翻译点与遴选时机、`data_log` 去向、死代码
清理、POI 悬空引用、编组构成落盘）：**在定稿前，不要按本设想动手改
`model/wagon.py` / `model/train_entity.py` / `model/session.py` 的结构**——只可做
与设想无关的独立小修（如 B5 自反断言）。
