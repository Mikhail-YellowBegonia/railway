...
——**✅ 设计已研讨完毕、规划已封口（2026-09-10）**，**下一步从 P0（分段器）开始实现**。
...
- **形态**：计划 = **有序条目 + 指令指针**（指针在控制车/车厢上）；**条目可携带
  "路径限定"**（锚点序列 + 控制点）——**锚点比目标级低一层**：只有条目携带逻辑，
  锚点解析之后**只是路径**（**不是**第二种条目）。
...
- **不变的底层**：执行层**零改动**——`route: list[DirectedEdge]` → 进路预约 → 授权 →
  ...
  只有 `GameLoop._find_route`，被右键与 K 连挂调用）。
